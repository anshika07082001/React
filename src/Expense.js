...
let sel = ["Groceries", "Veggies", "Travelling", "Miscellaneous"],
  sel2 = ["Expense", "Income"],
  grocery = [],
  veggies = [],
  travel = [],
...
const Expense = () => {
  ...
  const addHandler = (event) => {
    ...
    if (btnVal === "Add Transaction") {
      if (select2 === 1) {
        if (txt !== "" && amt !== "") {
          ...
          let exp = document.getElementById("money-minus").innerHTML;
      ...
      if (select2 === 2) {
        if (amt !== "") {
          let inc = document.getElementById("money-plus").innerHTML;
    ...
    let tot = document.getElementById("money-plus").innerHTML;
    let ex = document.getElementById("money-minus").innerHTML;
    let total = parseInt(tot) + parseInt(ex);
  ...
  return (
    <div className="container">
      <h1 style={{ color: "red" }}>Budget Planner</h1>
      <br />
      <h4>Your Balance</h4>
      ...
      <div className="total">
        <h4>Groceries</h4>
        <h4 id="gSum">Total=0</h4>
      </div>

      <ul className="list">
        {Groceries.map((object, i) => (
          <li className="minus" key={object.text} index={i} value={object.text}>
            {object.text}
      ...
      <div className="total">
        <h4>Veggies</h4>
        <h4 id="vSum">Total=0</h4>
      </div>
      <ul className="list">
        {Veggies.map((object, i) => (
          <li className="minus" key={object.text} index={i} value={object.text}>
            {object.text} <span>{object.money}</span>
            <button
              className="delete-btn"
              status="Veggies"
              onClick={deleteHandler}
      ...
      <div className="total">
        <h4>Miscellaneous</h4>
        <h4 id="mSum">Total=0</h4>
      </div>
      <ul className="list">
        {Miscllaneous.map((object, i) => (
          <li className="minus" index={i} key={object.text} value={object.text}>
            {object.text} <span>{object.money}</span>
            <button
              className="delete-btn"
      ...
      <form>
        <div>
          ...
          <input
            type="text"
            id="text"
            placeholder="Selected Categories Text..."
            ref={inpValText}
          />
        </div>
        <div>
